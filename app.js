...
function currentLevel(){for(let i=1;i<=100;i++)if(!state.completed.includes(i)&&isUnlocked(i))return i;for(let i=1;i<=100;i++)if(!state.completed.includes(i))return i;return 100}
...
function streak(){let dates=new Set(state.activity);let d=new Date();d.setHours(12,0,0,0);let s=0;const key=x=>`${x.getFullYear()}-${String(x.getMonth()+1).padStart(2,'0')}-${String(x.getDate()).padStart(2,'0')}`;for(;;){let k=key(d);if(dates.has(k)){s++;d.setDate(d.getDate()-1)}else{if(s===0){d.setDate(d.getDate()-1);let y=key(d);if(dates.has(y)){s++;d.setDate(d.getDate()-1);continue}}break}}return s}
...
function unlockCards(n){for(const c of DATA.cards.filter(x=>x.level===n)){if(!state.reviews[c.id]){let d=new Date();d.setHours(12,0,0,0);d.setDate(d.getDate()+1);const due=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;state.reviews[c.id]={interval:1,due,ease:2.2}}}}
