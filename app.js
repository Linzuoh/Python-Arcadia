...
function render(){const [r,a]=parseRoute();setNav(['lesson','exam'].includes(r)?'home':r);if(r==='home')home();else if(r==='lesson')lessonView(Number(a));else if(r==='review')reviewView();else if(r==='exams')examsView();else if(r==='exam')examView(Number(a));else if(r==='progress')progressView();else home();window.scrollTo(0,0);updateChrome()}
...
async function boot(){document.getElementById('boot').classList.add('hidden');document.getElementById('app').classList.remove('hidden');updateChrome();render();try{if(typeof loadPyodide!=='function')throw new Error('Biblioteca Pyodide não carregou');pyodide=await loadPyodide({indexURL:'https://cdn.jsdelivr.net/pyodide/v0.28.3/full/'});const warm=await pyodide.runPythonAsync('40 + 2');if(Number(warm)!==42)throw new Error('Teste interno do Python falhou');const el=document.getElementById('pyStatus');if(el){el.textContent='● Python pronto · v4';el.classList.add('ready')}}catch(e){console.error('Arcádia: falha ao iniciar Python',e);const el=document.getElementById('pyStatus');if(el){el.textContent='● Python indisponível';el.classList.remove('ready')}toast('Não consegui iniciar o Python. Recarregue a página.')}}
