...
function ensureV73State(){
  if(state.version73)return;

  // Remove registros antigos que eram somente falhas de teste automático.
  for(const [key,e] of Object.entries(state.errorNotebook||{})){
    if(e?.kind==='AssertionError' || /:AssertionError$/i.test(key)) delete state.errorNotebook[key];
  }
  state.customReviews=(state.customReviews||[]).filter(c=>!/assertionerror/i.test(c.id||''));

  // Os desafios N011/N012 mudaram para não cobrar listas antes da aula de listas.
  // Só apagamos rascunhos antigos se o nível ainda não foi concluído.
  if(!state.completed.includes(11) && /contagem\s*=\s*\[\]/.test(String(state.codes?.[11]||''))){
    delete state.codes[11];
    delete state.passed[11];
  }
  if(!state.completed.includes(12) && /multiplos_de_3\s*=\s*\[\]/.test(String(state.codes?.[12]||''))){
    delete state.codes[12];
    delete state.passed[12];
  }

  state.version73=true;
...
function recordError(n,error,code='',source='desafio'){
  const kind=errorKind(error,code),skill=skillForLevel(n);
  // AssertionError aqui é normalmente o teste automático dizendo apenas
  // "o resultado ainda não bateu com o contrato". Isso não é, por si só,
  // um padrão de erro do aluno e não deve poluir o Caderno.
  if(kind==='AssertionError'){
    return {count:0,ignored:true,kind,skill:skill?.id||'geral'};
  }
  const key=`${skill?.id||'geral'}:${kind}`;
...
ensureV73State();
applyCosmetics();document.getElementById('boot').classList.add('hidden');document.getElementById('app').classList.remove('hidden');updateChrome();render();
  document.getElementById('importInput').onchange=async e=>{const f=e.target.files[0];if(!f)return;try{state={...initial(),...JSON.parse(await f.text())};ensureV6State();
...
ensureV73State();
save();render();toast('Progresso importado')}catch{toast('Arquivo de progresso inválido')}};
